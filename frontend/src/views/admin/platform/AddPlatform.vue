<template>
  <div class="card flex-shrink-0 max-w-lg shadow-2xl bg-base-100 mx-auto">
    <div class="p-8 pb-0 flex items-center">
      ...
      <h1 class="text-4xl pl-5">Add platform</h1>
    </div>
    <div class="card-body">
      <ValidationForm @submit="savePlatform" :validation-schema="schema">
...
<script>
  ...
  export default {
    ...
    methods: {
      savePlatform (platform) {
        ...
        Platform.create(platform)
          .then(() => {
            this.$router.push({ name: 'admin-platforms' })
          })
          .catch(error => {
            this.loading = false
            this.message =
    ...
    mounted () {
      useHead({
        title: computed(() => 'Admin - Add platform | ' + process.env.VUE_APP_TITLE)
      })
    }
  }
</script>
