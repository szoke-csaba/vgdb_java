<template>
  <div class="card overflow-visible flex-shrink-0 max-w-lg shadow-2xl bg-base-100 mx-auto" v-if="currentGenre">
    <div class="p-8 pb-0 flex items-center">
      <router-link :to="{ name: 'admin-genres' }" class="text-4xl">
        <font-awesome-icon icon="fa-solid fa-arrow-left" />
      </router-link>
      <h1 class="text-4xl pl-5">Update genre</h1>
    </div>
    <div class="card-body">
      <ValidationForm @submit="updateGenre" :validation-schema="schema">
        <div class="form-control">
          <label for="name" class="label">
            <span class="label-text">Name</span>
          </label>
          <Field id="name" name="name" type="text" class="input input-bordered" v-model="currentGenre.name" />
